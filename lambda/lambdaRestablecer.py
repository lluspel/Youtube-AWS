...
def lambda_handler(event, context):
    try:
        with conn.cursor() as cur:
            ...
            conn.commit()
            cur.execute("UPDATE usuarios SET pwd='" +event['queryStringParameters']['pwd']+ 
                "' AND WHERE email='" + event['queryStringParameters']['email']+"'")
            conn.commit()
            cur.execute("SELECT usuario FROM usuarios WHERE email='" +
                event['queryStringParameters']['email']+"' AND pwd ='" +event['queryStringParameters']['pwd']+ 
                "'")
            conn.commit()
            for row in cur:
